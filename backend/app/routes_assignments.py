...
@router.post("")
def create_assignment(title: str, description: str, subject_id: int, faculty_id: int, db: Session = Depends(get_db)):
    faculty = db.get(User, faculty_id)
    if not faculty or faculty.role != "faculty":
        raise HTTPException(403, "Only faculty can create assignments")
    if not db.get(Subject, subject_id):
        raise HTTPException(404, "Subject not found")
    item = Assignment(title=title, description=description, subject_id=subject_id, faculty_id=faculty_id)
